...
# -----------------------------------------------------------------------------
# Athena Workgroup (patch 3)
# -----------------------------------------------------------------------------

variable "athena_bytes_scanned_cutoff" {
  description = "Per-query bytes-scanned limit for the FinOps Athena workgroup. Athena minimum is 10485760 (10MB). 10GB ~= $0.05."
  type        = number
  default     = 10737418240

  validation {
    condition     = var.athena_bytes_scanned_cutoff >= 10485760
    error_message = "Athena requires at least 10485760 (10 MB)."
  }
}
