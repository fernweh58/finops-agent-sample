...
locals {
  ...
  # Athena results location shared by the workgroup (patch 3) and the athena
  # Lambda so they never drift. When cur_athena_output_location is empty it
  # falls back to the CUR bucket's athena-results/ prefix.
  athena_output_location = var.cur_athena_output_location != "" ? var.cur_athena_output_location : "s3://${var.cur_bucket_name}/athena-results/"
  ...
  tool_schema_files = {
    ...
    "aws-api-cross-account-mcp"   = "aws_api_cross_account.json"
  }
  mcp_tool_schemas = {
...
locals {
  mcp_lambda_targets = [
    ...
    {
      name         = "aws-api-cross-account-mcp"
      description  = "Cross-account AWS API tools (read-only, payer account)"
      lambda_arn   = module.mcp_aws_api_cross_account.function_arn
      tool_schemas = local.mcp_tool_schemas["aws-api-cross-account-mcp"]
    },
  ]
}
...
module "mcp_athena" {
  ...
  environment_variables = {
    CUR_OUTPUT_LOCATION = local.athena_output_location
